...
def create_lef(mem):
    ...
    if num_rwport == 1:
        ...
        port_suffix = {
            n: (f"_{chr(n+ord('A'))}", "left" if n % 2 == 0 else "right")
    ...
    for i in range(int(bits)):
        for n in range(num_rwport):
            lef_add_pin(
                fid,
                mem,
                "w_mask_in%s[%d]" % (port_suffix[n][0], i),
                True,
                y_step,
                pin_pitch,
                port_suffix[n][1],
